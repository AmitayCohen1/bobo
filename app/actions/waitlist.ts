...
export async function createWaitlistEntry(
  ...
  await sql`
    INSERT INTO waitlist (product, variant_type, color, size, quantity, customer_name, phone, notes, heard_from)
    VALUES (${product}, ${variantType}, ${color}, ${size}, ${quantity}, ${name}, ${phone}, ${notes}, ${heardFrom})
  `;

  after(() => notifyNewWaitlistEntry({ product, variantType, color, size, quantity, name, phone, notes, heardFrom }));
...
export async function updateWaitlistContacted(formData: FormData): Promise<void> {
  const session = await getAdminSession();
  if (!session) throw new Error("unauthorized");
